"""Probe Noon API with correct auth to find warehouse code + test orders/stock."""
...
# 1. List orders (FBPI) — no warehouse code yet, try empty / PRJ245
now = datetime.now(timezone.utc)
after = (now - timedelta(days=30)).strftime("%Y-%m-%dT%H:%M:%SZ")
before = now.strftime("%Y-%m-%dT%H:%M:%SZ")

call("post", "FBPI list-orders (no warehouse)", f"{BASE}/fbpi/v1/fbpi-orders/list",
     json={"created_after": after, "created_before": before})

call("post", "FBPI list-orders (warehouse PRJ242545)", f"{BASE}/fbpi/v1/fbpi-orders/list",
     json={"warehouse_code": "PRJ242545", "created_after": after, "created_before": before})

# 2. Try stock/inventory endpoints
call("get",  "stock list",        f"{BASE}/stock/v1/stocks")
call("post", "stock list (POST)", f"{BASE}/stock/v1/stocks/list", json={})
call("get",  "warehouse list",    f"{BASE}/fbpi/v1/warehouses")
call("get",  "catalog products",  f"{BASE}/catalog/v1/products")
call("post", "catalog products",  f"{BASE}/catalog/v1/products/list", json={})
call("get",  "offer list",        f"{BASE}/offer/v1/offers")
call("post", "offer list",        f"{BASE}/offer/v1/offers/list", json={})
