"""Create all exports and poll for results."""
...
def create_export(category_code, params):
    r = session.post(f"{BASE}/impex/v1/export/create",
                     json={"export_category_code": category_code, "params": params}, timeout=30)
    print(f"Create {category_code}: [{r.status_code}] {r.json()}")
    return r.json().get("export_code")

def poll_export(export_code, label, max_wait=60):
    print(f"\nPolling {label} ({export_code})...")
    for i in range(12):
        time.sleep(5)
        r = session.get(f"{BASE}/impex/v1/export/status/{export_code}", timeout=30)
        d = r.json()
        ...
        if status in ("COMPLETED", "FAILED", "completed", "failed"):
            ...
            return d
        if status == "unknown" and r.status_code != 200:
...
# Catalog (already created EXP4R57MXRQP, check it)
cat_code = "EXP4R57MXRQP"
print(f"Existing catalog export: {cat_code}")

# Sales data
sales_code = create_export("noon_catalog_reports_productviewsandsalesdata", {
    "country": "ae", "from_date": from_date, "to_date": to_date, "lang": "en"
})

# Financial transactions
finance_code = create_export("noon_financeweb_transactionviewreportonitemlevel", {
...
# Poll all three
poll_export(cat_code, "catalog")
if sales_code:
    poll_export(sales_code, "sales data")
if finance_code:
    poll_export(finance_code, "financial transactions")
